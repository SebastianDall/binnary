...
def test_motifs_scored_in_bins(loaded_data):
    ...
    assert sorted(bin_motifs["motif_mod"].unique()) == sorted(["m1_a", "m2_a", "m3_a", "m6_a"])
    
    # motifs_scored_in_bins
    print(motifs_scored_in_bins)
    assert sorted(bin_motifs["motif_mod"].unique().tolist()) == sorted(motifs_scored_in_bins["motif_mod"].unique().tolist())
    assert motifs_scored_in_bins.shape[1] == 13 # number of columns
    # Assert contig 1 belongs to bin 1
    assert motifs_scored_in_bins.loc[motifs_scored_in_bins["contig"] == "contig_1", "bin"].unique() == "b1"
